...
RSpec.describe 'Blood Test Management' do
  ...
  it 'displays correct form elements for blood test creation', :js do
    expect(page).to have_select('lab_test_biomarker_id', wait: 10)
    ...
    expect(page).to have_selector("input[name='lab_test[unit]'][value='#{reference_range.unit}']",
                                  visible: false, wait: 5)
  end
  ...
  it 'creates a blood test with valid data', :js do
    ...
    expect(page).to have_content(/success|created/i, wait: 5)
